...
type TemplatePoolDao struct {
    mysqlModel.TemplatePoolModel
}
...
func (m *TemplatePoolDao) Update(args map[string]interface{}) error {
    mysqlManager := database.GetMysqlClient()
    err := m.Get()
    if err != nil {
        return err
...
func (m *TemplatePoolDao) Delete() error {
    mysqlManager := database.GetMysqlClient()
    err := m.Get()
    if err != nil {
        return err
